<!--
 * @Author: 曹捷
 * @Date: 2019-08-26 12:49:18
 * @LastEditors: 曹捷
 * @LastEditTime: 2019-08-26 13:33:22
...
<template>
  <view class="serach">
    <view :style="{'border-radius':radius+'px'}" class="content">
      <!-- HM修改 增加进入输入状态的点击范围 -->
      <view :class="{'center':mode === 2}" @click="getFocus" class="content-box">
        ...
        <text @click.stop="clear" class="icon icon-del" v-if="isDelShow"></text>
      </view>
      <view @click="search" class="serachBtn" v-show="(active&&show&&button === 'inside')||(isDelShow && button === 'inside')">搜索</view>
    </view>
    <view :class="{'active':show||active}" @click="search" class="button" v-if="button === 'outside'">
